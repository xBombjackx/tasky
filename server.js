...
twitchClient.on("message", (channel, tags, message, self) => {
  ...
  const isMod =
    tags.mod ||
    tags.username.toLowerCase() === TWITCH_CHANNEL_NAME.toLowerCase();
  ...
  if (command === "task") {
    ...
    if (taskDescription) {
      ...
      twitchClient.say(
        channel,
        `@${username}, your task has been submitted for approval!`,
      );
    }
  }
  if (command === "approve" && isMod) {
    ...
    if (userToApprove) {
      approveViewerTask(userToApprove).then((msg) =>
  ...
  if (command === "reject" && isMod) {
    ...
    if (userToReject) {
      rejectViewerTask(userToReject).then((msg) =>
        twitchClient.say(channel, msg),
      );
    }
  }
  if (command === "done") {
    updateViewerTaskStatus(username, true).then((msg) =>
      twitchClient.say(channel, msg),
    );
  }
  if (command === "undo") {
    updateViewerTaskStatus(username, false).then((msg) =>
      twitchClient.say(channel, msg),
    );
  }
});
