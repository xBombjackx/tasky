...
if (!notionApiKey || !streamerDatabaseId || !viewerDatabaseId) {
  // Check both IDs
  console.error(
...
async function queryAndLogTasks(databaseId, databaseName) {
  ...
  if (
    ...
    databaseResponse.data_sources.length === 0
  ) {
    console.error(
      `Error: Could not retrieve data source information for database "${databaseName}" (ID: ${databaseId}).`,
    );
    console.error(
      "Ensure the integration has access to the database and the ID is correct.",
    );
  ...
  // 4. Log the results
  console.log(
    `Found ${queryResponse.results.length} tasks in "${databaseName}":`,
  );
    ...
    queryResponse.results.forEach((page) => {
      ...
      if (page.object === "page" && "properties" in page) {
        ...
        if (
          ...
          console.log(
            `- Task found, but property named "${titlePropertyName}" of type "title" was not found or is empty.`,
          );
...
async function runAllQueries() {
  ...
  try {
    ...
    if (error.code && error.message) {
      // Check if it's likely a NotionClientError
